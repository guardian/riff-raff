...
class ArtifactHousekeepingTest
    extends AnyFlatSpec
    with Matchers
    with MockitoSugar {
  ...
  private def mockListObjectsV2Result(
    ...
    val s3Objects: List[S3Object] = objectSummaries.map { obj =>
      S3Object
        .builder()
        .key(obj.key)
        .lastModified(obj.lastModified.toDate.toInstant)
        .build()
    }
  ...
  val oldDeploys: List[Record] = List.tabulate(6)(n =>
    fixtureRecord(oldDate.plusHours(n), "PROD", s"2$n")
  ) ++ List.tabulate(6)(n =>
    fixtureRecord(oldDate.plusHours(n), "CODE", s"1$n")
  )
  val newDeploys: List[Record] = List.tabulate(6)(n =>
    fixtureRecord(newDate.plusHours(n), "PROD", s"1$n")
  ) ++ List.tabulate(6)(n =>
    fixtureRecord(newDate.plusHours(n), "CODE", s"2$n")
  )
  ...
  "getProjectNames" should "make a single listObjectsV2 request and return the project names" in {
    ...
    val listObjectsResult: ListObjectsV2Response = ListObjectsV2Response
      .builder()
      .commonPrefixes(
        List(
          CommonPrefix.builder().prefix("project-name-1/").build(),
          CommonPrefix.builder().prefix("project-name-2/").build(),
          CommonPrefix.builder().prefix("project-name-3/").build()
        ).asJava
      )
      .build()

    when(
      artifactClientMock.listObjectsV2(any[ListObjectsV2Request])
    ) thenReturn listObjectsResult

    val result =
      ArtifactHousekeeping.getProjectNames(artifactClientMock, "bucket-name")
    verify(artifactClientMock, times(1)).listObjectsV2(
      any[ListObjectsV2Request]
    )
  ...
  "getBuildIds" should "make a single listObjectsV2 request and return the build IDs" in {
    ...
    val listObjectsResult: ListObjectsV2Response = ListObjectsV2Response
      ...
      .commonPrefixes(
        List(
          ...
          CommonPrefix.builder().prefix("project-name/12/").build()
        ).asJava
      )
      .build()
    when(
    ...
    val result = ArtifactHousekeeping.getBuildIds(
      artifactClientMock,
      "bucket-name",
      "project-name"
    )
  ...
  "getBuildIdsToKeep" should "find the most recent builds for each stage" in {
    ...
    when(
      deploymentsMock.getDeploys(
        any[Option[DeployFilter]],
        any[PaginationView],
        any[Boolean]
      )
    ) thenReturn
    ...
    val result = ArtifactHousekeeping.getBuildIdsToKeep(
      deploymentsMock,
      "testProject",
      50,
      5
  ...
  it should "find the older build that has been recently deployed" in {
    val deploymentsMock = mock[Deployments]
    val oldBuildToProd =
      List(fixtureRecord(newDate.plusHours(23), "PROD", "10"))
    when(
      deploymentsMock.getDeploys(
        any[Option[DeployFilter]],
        any[PaginationView],
  ...
  "getObjectsToTag" should "only return objects that are older than the configured date" in {
    ...
    when(
      artifactClientMock.listObjectsV2(any[ListObjectsV2Request])
    ) thenReturn mockListObjectsV2Result(
      List.tabulate(3)(n =>
        ObjectSummary(s"object-x$n", "project-name", oldDate.plusDays(n))
      ) ++
        List.tabulate(3)(n =>
          ObjectSummary(s"object-z$n", "project-name", newDate.plusDays(n))
        )
    )

    val result = ArtifactHousekeeping.getObjectsToTag(
      artifactClientMock,
      "bucket-name",
      "project-name",
      "12",
