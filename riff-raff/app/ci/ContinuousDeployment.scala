...
class ContinuousDeployment(
  ...
  def cdConfigs: List[ContinuousDeploymentConfig] = retryUpTo(5)(
    continuousDeploymentConfigRepository.getContinuousDeploymentList()
  ).getOrElse {
...
object ContinuousDeployment extends Logging with Retriable {
  ...
  def getDeployParams(
      configBuildTuple: (ContinuousDeploymentConfig, CIBuild)
  ): DeployParameters = {
