...
trait QueryStringBuilder {
  ...
  def queryString = queryStringParams
    .map { case (k, v) =>
...
case class DeployFilter(
  ...
  def withHasWarnings(hasWarnings: Option[Boolean]) =
    this.copy(hasWarnings = hasWarnings)
...
trait Pagination extends QueryStringBuilder {
  ...
  def lowerBound: Int = math.max(
    1,
    pageCount
      .map(pageCount => math.min(page - 2, pageCount - 4))
      .getOrElse(page - 4)
  )
  def upperBound: Int = pageCount
...
case class DeployFilterPagination(
    filter: DeployFilter,
    pagination: PaginationView,
    itemCount: Option[Int] = None
) extends QueryStringBuilder
    with Pagination {
  lazy val queryStringParams =
    filter.queryStringParams ++ pagination.queryStringParams
  ...
  def withStatus(status: Option[RunState]) =
...
object DeployFilterPagination {
  def fromRequest(implicit r: RequestHeader): DeployFilterPagination = {
    DeployFilterPagination(
      DeployFilter.fromRequest.getOrElse(DeployFilter()),
