...
case class ParamDoc(name: String, documentation: String, default: Option[String])
...
object DeployTypeDocs {
  ...
  def generateDocs(deploymentTypes: Seq[DeploymentType]): Seq[(DeploymentType, DeployTypeDocs)] = {
    deploymentTypes.sortBy(_.name).map { dt =>
      ...
      val deploymentPackage = DeploymentPackage("<packageName>",App("<app>"),Map.empty,dt.name,
        S3Path("<bucket>", "<prefix>"), deploymentTypes)
