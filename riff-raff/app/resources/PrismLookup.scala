...
class PrismLookup(
      ...
      case key @ KeyPattern(service) =>
        data.datum(key, app, stage, stack).flatMap { data =>
          if (service.endsWith("role")) {
            Some(
              service -> ApiRoleCredentials(service, data.value, data.comment)
            )
          } else {
            secretProvider.lookup(service, data.value).map { secret =>
              service -> ApiStaticCredentials(
                service,
                data.value,
                secret,
                data.comment
  ...
  object prism extends Logging {
    def get[T](path: String, retriesLeft: Int = 5)(block: JsValue => T): T = {
      val result =
        wsClient.url(s"${config.lookup.prismUrl}$path").get().map(_.json).map {
          json =>
            block(json)
        }
      try {
        ...
        case NonFatal(e) =>
          log.warn(
            s"Call to prism failed ($path; $retriesLeft retries left)",
            e
          )
  ...
  def lastUpdated: DateTime = prism.get("/sources?resource=instance") { json =>
    val sourceCreatedAt = json \ "data" match {
      case JsDefined(JsArray(sources)) =>
        sources.map { source => (source \ "state" \ "createdAt").as[DateTime] }
  ...
  def data = new DataLookup {
    ...
    def datum(
      ...
      val query =
        s"/data/lookup/${key.urlEncode}?stack=${stack.name.urlEncode}&app=${app.name.urlEncode}&stage=${stage.name.urlEncode}"
  ...
  def hosts = new HostLookup {
    ...
    def get(
      ...
      val query =
        s"/$entity?_expand&stage=${parameters.stage.name.urlEncode}&stack=${stack.name.urlEncode}&app=${app.name.urlEncode}"
  ...
  private def get(
    ...
    val params: Seq[(String, String)] =
      tags.map { case (key, value) =>
        s"tags.${key.urlEncode}" -> value
      }.toSeq ++
        accountNumber.map(acc => "meta.origin.accountNumber" -> acc) ++
        Map("region" -> region, "state" -> "available")
    val paramsQueryString =
      params.map { case (k, v) => s"$k=${v.urlEncode}" }.mkString("&")
