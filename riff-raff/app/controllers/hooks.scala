...
case class HookForm(id:UUID, projectName: String, stage: String, url: String, enabled: Boolean, method: HttpMethod)
...
object Hooks extends Controller with Logging with LoginActions {
  ...
  def form = AuthAction { implicit request =>
    Ok(views.html.hooks.form(request,hookForm.fill(HookForm(UUID.randomUUID(),"","","",enabled=true, GET))))
  }
  def save = AuthAction { implicit request =>
    hookForm.bindFromRequest().fold(
      formWithErrors => Ok(views.html.hooks.form(request,formWithErrors)),
      f => {
        val config = HookConfig(f.id,f.projectName,f.stage,f.url,f.enabled,new DateTime(),request.user.fullName, f.method)
  ...
  def edit(id: String) = AuthAction { implicit request =>
    ...
    Persistence.store.getPostDeployHook(uuid).map{ hc =>
      Ok(views.html.hooks.form(request,hookForm.fill(HookForm(hc.id,hc.projectName,hc.stage,hc.url,hc.enabled, hc.method))))
