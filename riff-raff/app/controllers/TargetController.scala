...
class TargetController(config: Config,
                       menu: Menu,
  ...
  def findAppropriateDeploy(region: String, stack: String, app: String, stage: String) = authAction { request =>
    ...
    targetIds match {
      ...
      case Nil => NotFound(views.html.deployTarget.noMatchForTarget(config, menu)(target, request))
      case multiple => Ok(views.html.deployTarget.selectTarget(config, menu)(target, multiple.sortBy(-_.lastSeen.getMillis), stage, request))
    }
  }
