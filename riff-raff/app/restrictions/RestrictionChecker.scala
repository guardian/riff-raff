...
object RestrictionChecker {
  ...
  /** Method that checks whether the supplied config is editable by the supplied
    * source and reports and error reason if not.
    * @param currentConfig
    *   The config to check - note that when a config is being checked this must
    *   be the original config not the updated config
    * @param identity
    *   The identity of the user trying to make the edit
    * @return
    *   Either true or a string containing the reason it is not editable
    */
  def isEditable(
  ...
  def sourceMatches(
    ...
    source match {
      case ContinuousDeploymentRequestSource | ScheduleRequestSource =>
        config.continuousDeployment
      case UserRequestSource(identity) =>
        config.allowlist.contains(identity.email)
      case _ => false
    }
  }
