  ...
  lazy val stage: String = {
    val theStage = Try(EC2MetadataUtils.getInstanceId) match {
      case Success(instanceId) if instanceId != null =>
        ...
        val ec2Client =
          Regions.getCurrentRegion.createClient(classOf[AmazonEC2Client], credentialsProviderChain(None, None), null)
        try {
          val describeTagsResult = ec2Client.describeTags(request)
          describeTagsResult.getTags.asScala.collectFirst { case t if t.getKey == "Stage" => t.getValue }
