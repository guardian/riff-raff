...
class SummariseDeploysHousekeeping(config: Config, datastore: DataStore)
    extends Lifecycle
    with Logging {
  lazy val maxAgeDays = config.housekeeping.summariseDeploysAfterDays
  lazy val housekeepingTime =
    new LocalTime(config.housekeeping.hour, config.housekeeping.minute)
  ...
  def summariseDeploys(): Int = {
    ...
    val deploys = datastore.getCompleteDeploysOlderThan(
      maxAgeThreshold.toDateTimeAtStartOfDay
    )
