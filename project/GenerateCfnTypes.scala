...
object GenerateCfnTypes {
   ...
   */
  def generate(packageName: String, objectName: String, regions: List[String]): String = {

    s"""
      |// This file is generated by ${getClass.getName}
      ...
      |    ${getTypes(regions).map { case (region, types) => s""""$region" ->
      |      Set(
  ...
  private def getTypes(regions: List[String]) = {
    regions.map { region =>
      lazy val client = CloudFormationClient
        .builder()
        .credentialsProvider(ProfileCredentialsProvider.builder().profileName("membership").build())
