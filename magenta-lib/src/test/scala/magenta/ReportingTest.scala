...
class ReportingTest extends AnyFlatSpec with Matchers {
  ...
  it should "build a one-op deploy report" in {
    val stacks = startDeployWrapper :: wrapper(
      ...
      stack(infoMsg, deploy)
    ) :: finishDeployWrapper :: Nil
    val report = DeployReport(stacks)

    report.isRunning should be(false)
    report.size should be(2)
    report shouldBe tree(
      deployMessageId,
      startDeploy,
      finishDep,
  ...
  it should "render a report as text" in {
    ...
    tree.render.mkString(", ") should be(
      """:Deploy(DeployParameters(Deployer(Test reports),Build(test-project,1),Stage(CODE),All,MostlyHarmless)) [Completed], 1:Info($ echo hello) [Completed], 1.1:CommandOutput(hello) [NotRunning], 1.2:Verbose(return value 0) [NotRunning]"""
    )
  }
  ...
  def tree(
    ...
    DeployReportTree(
      MessageState(startMessage, finishMessage, testTime, id),
      trees.toList
    )
  }
  def tree(
    ...
    DeployReportTree(
      MessageState(startMessage, failMessage, testTime, id),
      trees.toList
    )
  }

  val parameters = DeployParameters(
    Deployer("Test reports"),
