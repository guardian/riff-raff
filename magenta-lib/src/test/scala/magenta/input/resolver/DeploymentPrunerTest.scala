...
class DeploymentPrunerTest extends AnyFlatSpec with Matchers {
  ...
  "StackAndRegion pruner" should "return None when either the region or stack isn't specified" in {
    ...
    testStackAndRegionPruner(
      testDeployment.copy(regions = NEL.of("differentRegion"))
    ) shouldBe None
    testStackAndRegionPruner(
      testDeployment.copy(
        stacks = NEL.of("differentStack"),
        regions = NEL.of("differentRegion")
      )
    ) shouldBe None
  }
  ...
  "Keys pruner" should "return None when the name, action, region or stack doesn't match" in {
    ...
    testKeysPruner(
      testDeployment.copy(actions = NEL.of("differentAction"))
    ) shouldBe None
    testKeysPruner(
      testDeployment.copy(stacks = NEL.of("differentStack"))
    ) shouldBe None
