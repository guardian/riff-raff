...
case class DeployReporter(
  ...
  def taskList(tasks: List[Task]): Unit = {
    DeployReporter.send(this, Message.TaskList(tasks))
  }
  def info(message: String): Unit = {
    DeployReporter.send(this, Message.Info(message))
  }
  def infoContext[T](message: String)(block: DeployReporter => T): T = {
    DeployReporter.sendContext(this, Message.Info(message))(block)
  }
  def commandOutput(message: String): Unit = {
    DeployReporter.send(this, Message.CommandOutput(message))
  }
  def commandError(message: String): Unit = {
    DeployReporter.send(this, Message.CommandError(message))
  }
  def verbose(message: String): Unit = {
    DeployReporter.send(this, Message.Verbose(message))
  }
...
object DeployReporter {
  ...
  def startDeployContext(reporter: DeployReporter) =
    pushContext(Message.Deploy(reporter.messageContext.parameters), reporter)
  ...
  def finishContext(currentReporter: DeployReporter): Option[DeployReporter] = {
    currentReporter.messageStack.headOption.foreach(msg =>
      send(currentReporter, ContextMessage.FinishContext(msg))
    )
  ...
  def failContext(currentReporter: DeployReporter): Option[DeployReporter] = {
    currentReporter.messageStack.headOption.foreach(msg =>
      send(currentReporter, ContextMessage.FailContext(msg))
    )
    currentReporter.previousReporter
  }
  def failContext(
  ...
  private def sendContext[T](currentReporter: DeployReporter, message: Message)(
      block: DeployReporter => T
  ): T = {
...
case object ContextMessage
    extends Enum[ContextMessage]
    with PlayJsonEnum[ContextMessage] {
