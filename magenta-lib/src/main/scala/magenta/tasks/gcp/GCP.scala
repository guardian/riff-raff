...
import com.google.api.services.deploymentmanager.{
  DeploymentManager,
  DeploymentManagerScopes
}
...
object GCP {
  lazy val httpTransport: ApacheHttpTransport =
  ...
  object credentials {
    def getCredentials(keyring: KeyRing): Option[GoogleCredential] = {
      keyring.apiCredentials.get("gcp").collect {
  ...
  object DeploymentManagerApi extends Loggable {
    ...
    case class InProgress(id: String, project: String, progress: Integer)
        extends DMOperation
    case class Success(id: String, project: String) extends DMOperation
    case class Failure(id: String, project: String, error: List[Errors])
        extends DMOperation
    ...
    object DMOperation {
      def apply(operation: Operation, project: String): DMOperation = {
        ...
        (
          Option(operation.getError).flatMap(e => Option(e.getErrors)),
          operation.getStatus
        ) match {
          case (_, "PENDING") =>
            InProgress(operationName, project, operation.getProgress)
          case (_, "RUNNING") =>
            InProgress(operationName, project, operation.getProgress)
          case (Some(errors), "DONE") =>
            Failure(operationName, project, errors.asScala.toList)
          case (None, "DONE") => Success(operationName, project)
          case (_, status) =>
            throw new IllegalArgumentException(
              s"Unexpected deployment manager operation status $status"
            )
        }
      }
    }

    case class DeploymentBundle(
        configPath: String,
        config: String,
    ...
    def toTargetConfiguration(bundle: DeploymentBundle): TargetConfiguration = {
      ...
      new TargetConfiguration()
        ...
        .setImports(imports.toList.asJava)
    }

    def get(client: DeploymentManager, project: String, name: String)(
      ...
      api.retryWhen500orGoogleError(reporter, "Deployment Manager get") {
        ...
        case gjre: GoogleJsonResponseException
            if gjre.getDetails.getCode == 404 =>
          None
      }
    }
    ...
    def operationStatus(client: DeploymentManager, operation: DMOperation)(
        reporter: DeployReporter
    ): Result[DMOperation] = {
      api
        .retryWhen500orGoogleError(
      ...
      val resquestInitialiser = new HttpRequestInitializer {
        override def initialize(request: HttpRequest): Unit = {
          ...
          request.setConnectTimeout(
            0
          ) // Infinite timouts prevents  java.net.SocketException Connection reset on gcs reads
          request.setReadTimeout(0)
        }
      }

      new Storage.Builder(httpTransport, jsonFactory, resquestInitialiser)
        .setRootUrl("https://storage.googleapis.com/")
        .build()
    }
  }

  object api {
    def retryWhen500orGoogleError[T](
        reporter: DeployReporter,
...
object GCS {
  def withGCSClient[T](keyRing: KeyRing, resources: DeploymentResources)(
      block: Storage => T
  ): T = block(
    GCP.StorageApi.client(
