...
object GCPRetryHelper extends Loggable {
  ...
  /** A task that has potentially been retried, with accumulated errors. There
    * are 3 cases:
    *   1. The task failed 1 or more times, and the final result is an error.
    *      - This is represented as {{{Left(NonEmptyList(errors))}}} 2. The task
    *        failed 1 or more times, but eventually succeeded.
    *      - This is represented as {{{Right(List(errors), A)}}} 3. The task
  ...
  def retryExponentially[T](
    ...
    retryInternal(
      reporter,
      exponentialBackOffIntervals,
      pred,
      failureLogMessage
    )(op)
  }

  /** will retry at the given interval until success or the overall timeout has
    * passed
    * @param pred
  ...
  def addJitter(
      baseTime: FiniteDuration,
      maxJitterToAdd: Duration
  ): FiniteDuration = {
    baseTime + ((scala.util.Random
      .nextFloat() * maxJitterToAdd.toNanos) nanoseconds)
  }
