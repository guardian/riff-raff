...
case class GCSUpload(
    ...
    withClientFactory: (KeyRing, DeploymentResources) => (
        Storage => Unit
    ) => Unit = GCS.withGCSClient[Unit]
) extends Task
    with Loggable {

  private val PublicAcl = Arrays.asList(
    new ObjectAccessControl().setEntity("allUsers").setRole("READER")
  )

  lazy val objectMappings: Seq[(S3Object, GCSPath)] = paths flatMap {
    case (file, targetKey) =>
      resolveMappings(file, targetKey, gcsTargetBucket.name)
  }
  ...
  lazy val transfers: Seq[StorageObjectTransfer] = objectMappings.map {
    case (source, target) =>
      val storageObject = new StorageObject()
        .setBucket(gcsTargetBucket.name)
        .setName(target.key)
...
object GCSUpload {
  def prefixGenerator(
    ...
    (stack.map(_.name) :: stage.map(_.name) :: packageName :: Nil).flatten
      .mkString("/")
  }
