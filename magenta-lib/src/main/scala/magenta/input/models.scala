...
object ConfigErrors {
  def apply(context: String, message: String): ConfigErrors = ConfigErrors(
    ConfigError(context, message)
  )
  def apply(error: ConfigError): ConfigErrors = ConfigErrors(NEL.of(error))
  implicit val sg = new Semigroup[ConfigErrors] {
    def combine(x: ConfigErrors, y: ConfigErrors): ConfigErrors = ConfigErrors(
      x.errors.concatNel(y.errors)
    )
  }
}

/** Represents a deployment shown to a user. These are flattened when the graph
  * is built so we only display a deployment that has one each of name, action,
  * region and stack. This also limits the complexity of the selection that
  * needs to be done when deploying. A deployment key can be represented as a
  * string and there are some methods for doing that at the package level.
  */
case class DeploymentKey(
    name: String,
    action: String,
    stack: String,
    region: String
...
object DeploymentKey {
  ...
  /** Turn a deployment into a deployment key - WARNING: this doesn't make any
    * checks about the number of elements but will only work predictably with
    * one element in each of actions, stacks and regions
    */
...
/** Represents entries for deployments and templates in a riff-raff.yml.
  * Deployments and deployment templates have the same structure so this class
  * can represent both.
  *
  * @param `type`
  *   The type of deployment to perform (e.g. autoscaling, s3).
  * @param template
  *   Name of the custom deploy template to use for this deployment.
  * @param stacks
  *   Stack tags to apply to this deployment. The deployment will be executed
  *   once for each stack.
  * @param regions
  *   A list of the regions in which this deploy will be executed. Defaults to
  *   just 'eu-west-1'
  * @param actions
  *   Override the list of actions to execute for this deployment type.
  * @param app
  *   The `app` tag to use for this deployment. By default the deployment's key
  *   is used.
  * @param contentDirectory
  *   The path where this deployment is found in the build output. Defaults to
  *   app.
  * @param dependencies
  *   This deployment's execution will be delayed until all named dependencies
  *   have completed. (Default empty)
  * @param parameters
  *   Provides additional parameters to the deployment type. Refer to the
  *   deployment types to see what is required.
  */
case class DeploymentOrTemplate(
    ...
    stacks: Option[List[String]],
    regions: Option[List[String]],
    allowedStages: Option[
      List[String]
    ], // While part of the model, this is always considered globally and should not be specified in a template
