...
object DeploymentResolver {
  ...
  private[input] def resolveDeployment(label: String,
                                       ...
                                       globalRegions: Option[List[String]]): Either[ConfigError, Deployment] = {
    for {
  ...
  private[input] def applyTemplates(
      ...
      templates: Option[Map[String, DeploymentOrTemplate]]): Either[ConfigError, DeploymentOrTemplate] = {
      ...
      case Some(parentTemplateName) =>
        for {
          parentTemplate <- templates
            .flatMap(_.get(parentTemplateName))
            .toRight(ConfigError(templateName, s"Template with name $parentTemplateName does not exist"))
            .right
  ...
  private[input] def validateDependencies(
      ...
      allDeployments: List[(String, DeploymentOrTemplate)]): Either[ConfigError, Deployment] = {
